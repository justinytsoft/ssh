        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    var path = list[i].url + "";
                    var start = path.indexOf("ueditor");
                    var realPath = path.substring(start, path.length);
                    
                    img.setAttribute('src', urlPrefix + realPath + (realPath.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlPrefix + realPath);
